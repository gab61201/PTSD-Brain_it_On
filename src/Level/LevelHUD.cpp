...
LevelHUD::LevelHUD(LevelId levelId, const std::string& targetText, int strokeLimit) {
    ...
    auto levelFrame = UI::Element::Image(
        Path::LevelFrame, {LEFT_UI_X, 280.0f}, {0.2f, 0.2f}, Layer::UIOutline);
    m_Renderer.AddChild(levelFrame);

    // 2. 關卡編號文字
    auto levelNumText = UI::Element::Text(
        std::to_string(static_cast<int>(levelId) + 1), 36, {LEFT_UI_X + 8, 290.0f}, WHITE, Layer::UIElement);
    m_Renderer.AddChild(levelNumText);

    // 3. 計時器鬧鐘圖示
    auto alarmIcon = UI::Element::Image(
        Path::Alarm, {LEFT_UI_X, 160.0f}, {0.12f, 0.12f}, Layer::UIElement);
    m_Renderer.AddChild(alarmIcon);

    // 4. 計時器文字
    m_TimerObject = UI::Element::Text(
        "0.0", 26, {LEFT_UI_X + 7, 110.0f}, WHITE, Layer::UIElement);
    m_Renderer.AddChild(m_TimerObject);

    // 5. 提示文字
    m_TargetTextObject = UI::Element::Text(targetText, 30, {0.0f, 250.0f}, WHITE, Layer::UIElement);
    m_Renderer.AddChild(m_TargetTextObject);

    // 6. 限制筆劃次數圖示
    auto strokeLimitIcon = UI::Element::Image(
        Path::StrokeLimit, {LEFT_UI_X - 5, 50.0f}, {0.12f, 0.12f}, Layer::UIElement);
    ...
    m_StrokeLimitObject = UI::Element::Text(
        ...
        24, {LEFT_UI_X + 8.0f, 7.0f}, WHITE, Layer::UIElement);
    m_Renderer.AddChild(m_StrokeLimitObject);

    // 8. 接觸倒數計時器（初始為隱藏）
    m_ContactTimerObject = UI::Element::Text(
        "0", 256, {0.0f, 0.0f}, WHITE, Layer::UIElement);
