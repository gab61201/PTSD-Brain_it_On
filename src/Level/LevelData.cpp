...
LevelData data;
    ...
    // ==========================================
    auto floorPart = std::make_shared<BaseObject>(
        ShapeType::RECTANGLE,
        glm::vec2(800.0F, 50.0F), // 寬 800 像素，高 50 像素
        glm::vec2(0.0F, 0.0F)     // 相對於父物件中心的偏移量為 0
    );
    auto floorComp = std::make_shared<CompositeObject>(
        std::vector<std::shared_ptr<BaseObject>>{floorPart},
        BodyType::STATIC,         // 靜態剛體 (不受重力影響)
    ...
    // 2. 建立「動態掉落的方塊」
    // ==========================================
    auto boxPart = std::make_shared<BaseObject>(
        ShapeType::RECTANGLE,
        glm::vec2(60.0F, 60.0F),  // 60x60 像素的方塊
        glm::vec2(0.0F, 0.0F)
    );
    auto boxComp = std::make_shared<CompositeObject>(
        std::vector<std::shared_ptr<BaseObject>>{boxPart},
        BodyType::DYNAMIC,        // 動態剛體 (會掉落)
    ...
    // ==========================================
    auto circlePart = std::make_shared<BaseObject>(
        ShapeType::CIRCLE,
        glm::vec2(50.0F, 50.0F),  // 直徑 50 像素 (半徑 25)
        glm::vec2(0.0F, 0.0F)
    );
    auto circleComp = std::make_shared<CompositeObject>(
        std::vector<std::shared_ptr<BaseObject>>{circlePart},
        BodyType::DYNAMIC,        // 動態剛體 (會掉落)
    ...
    // 4. 將所有組合件打包，並初始化物理世界
    // ==========================================
    std::vector<std::shared_ptr<CompositeObject>> objects = {
        floorComp, boxComp, circleComp
    };
    
    // 實例化物理世界 (建構子會自動將這些 objects 透過 AttachToWorld 掛載到 Box2D)
    data.world = std::make_shared<PhysicalWorld>(objects);
